  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-2 bg-white">
      ...
      <div className="grid grid-cols-4 grid-rows-1 text-gray-500 pt-1">
        ...
        <button className="text-lg flex-1 hover:font-bold hover:text-yellow-400">
          Catégories
        </button>
        <button className="flex-1 text-lg hover:font-bold hover:text-yellow-400">
          Pays
        </button>
        <button className="text-lg flex-1 hover:font-bold hover:text-yellow-400">
          Voyages
        ...
        {travelsList &&
          travelsList.map((travel: any) => {
            return (
              ...
              >
                <img
                  src={travel.picture_travel}
                  className="flex absolute w-full h-56 object-cover rounded-t-md "
                  onClick={() => {
                ...
                <div className="bg-black flex w-full bg-opacity-60 absolute pl-4 pt-2 pb-4 top-28">
                  <div className="TitleAndDate flex flex-col w-3/4">
                    ...
                    <p className="flex text-left text-sm italic font-light">
                      du{" "}
                      {new Date(travel.datestart_travel).toLocaleDateString(
                        "FR"
                      )}{" "}
                      au{" "}
                      {new Date(travel.dateend_travel).toLocaleDateString("FR")}
                    </p>
                  </div>
                  ...
                  <div className="CategoryAndPrice">
                    {travel.AvCategory.map((category: any) => (
                      <div className="Category flex px-2 pt-1">
                        <p
                          className="flex text-right text-sm text-yellow-400"
                          key={category.id}
                        >
